...
const { COLS, NUMBERS, SQUARE_LENGTH, GRID_LENGTH, SQUARE_ORDER } = config_json_1.default;
const { availableNumbers, unsetIndices, missingInSquare, indexToSquare, indicesForSquare, getValuesPerIndex, getIndicesToValues, valuesInRow, valuesInCol, indexToRow, indexToCol, horizontalIndices, verticalIndices, nonOrthogonalIndices, } = GridUtils_1.default;
const { rand, printGrid, shuffle, orderByValues, getIndexWithFewestAvailableValues, } = Utils_1.default;
function solveCollisions(values, square, grid) {
        ...
        for (const value of values) {
            ...
            if (valInCol && valInRow) {
                ...
                for (const otherIndex of otherIndices) {
                    const isValueInOtherCol = valuesInCol(indexToCol(otherIndex), grid).includes(value);
                    const isValueInOtherRow = valuesInRow(indexToRow(otherIndex), grid).includes(value);
                    if (isValueInOtherCol || isValueInOtherRow)
                        continue;
                    const otherValue = grid[otherIndex];
                    const isOtherValueInCol = valuesInCol(indexToCol(index), grid).includes(otherValue);
                    const isOtherValueInRow = valuesInRow(indexToRow(index), grid).includes(otherValue);
                    if (isOtherValueInRow || isOtherValueInCol)
                        continue;
                ...
                for (const otherIndex of otherIndices) {
                    const isValueInOtherRow = valuesInRow(indexToRow(otherIndex), grid).includes(value);
                    if (isValueInOtherRow)
                        continue;
                    const otherValue = grid[otherIndex];
                    const isOtherValueInRow = valuesInRow(indexToRow(index), grid).includes(otherValue);
                    if (isOtherValueInRow)
                        continue;
                    indexToChangeWith = otherIndex;
                    break;
                }
                ...
                for (const otherIndex of otherIndices) {
                    const isValueInOtherCol = valuesInCol(indexToCol(otherIndex), grid).includes(value);
                    if (isValueInOtherCol)
                        continue;
                    const otherValue = grid[otherIndex];
                    const isOtherValueInCol = valuesInCol(indexToCol(index), grid).includes(otherValue);
                    if (isOtherValueInCol)
                        continue;
                    indexToChangeWith = otherIndex;
                    break;
                }
...
function fillInSquare(square, grid) {
    const indices = indicesForSquare(square);
    ...
    while (valuesToFillIn.length > 0) {
        ...
        const index = getIndexWithFewestAvailableValues(indicesToValues[value], valuesPerIndex);
        if (index !== undefined) {
            grid[index] = value;
        }
        else {
    ...
    while (true) {
        ...
        for (let index = 0; index < SQUARE_ORDER.length; index++) {
            ...
            if (problematicValues.length > 0) {
                const collisionSolved = solveCollisions(problematicValues, square, grid);
                if (!collisionSolved) {
                    valid = false;
                    break;
                }
